...
@snap[west]
__Класс(англ. Class)__ - это описание или чертеж некоторой информативной сущности для создания __экземпляров(англ. Instance)__ этого класса. Также подходящей трактовкой этого понятия является ТИП данных. Мы описываем наш собственный тип и используем отдельные переменные(которые хранят экземпляры), которые уже хранят конкретные значения. Экземпляр это уже минимальная информативная еденица в нашей коде
@snapend
